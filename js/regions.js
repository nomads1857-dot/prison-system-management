var statesLGAs = {
    ...
    "Ebonyi": ["Abakaliki", "Afikpo North", "Afikpo South", "Ebonyi", "Ezza North", "Ezza South", "Ishiellu", "Ivo", "Izzi", "Ohaukwu", "Onicha"],
...
function print_state(state_id) {
    ...
    for (var state in statesLGAs) {
        option_str.options[option_str.length] = new Option(state, state);
    }
}

function print_lga(lga_id, state) {
    var option_str = document.getElementById(lga_id);
    option_str.length = 0;
    option_str.options[0] = new Option('Select LGA', '');
    option_str.selectedIndex = 0;
    var lgas = statesLGAs[state];
    if (lgas) {
        for (var i = 0; i < lgas.length; i++) {
            option_str.options[option_str.length] = new Option(lgas[i], lgas[i]);
        }
    }
}